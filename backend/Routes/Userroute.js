...
const CashBonusModel = require("../Models/CashBonusModel");
...
// ======================== USER CASH BONUS ROUTES ========================

// @route   GET /api/user/cash-bonus/list
// @desc    Get all cash bonuses for logged in user
// @access  Private (User only)
user_route.get("/cash-bonus/list/:userid",async (req, res) => {
  try {
    const userId = req.params.userid;
    console.log("res",req.body.userId)
    // Find all cash bonuses where user is assigned
    const bonuses = await CashBonusModel.find({
      "users.userId": userId
    }).sort({ createdAt: -1 });
    ...
    const userBonuses = bonuses.map(bonus => {
      ...
      if (!bonus.noExpiry && bonus.expiresAt) {
        isExpired = new Date() > new Date(bonus.expiresAt);
      }
      ...
      return {
        ...
        amount: bonus.amount,
        expiresAt: bonus.expiresAt,
        noExpiry: bonus.noExpiry || false,
        bonusType: bonus.bonusType,
        occasion: bonus.occasion,
        userStatus: userBonus?.status,
        claimedAt: userBonus?.claimedAt,
        isExpired: isExpired,
        canClaim: userBonus?.status === "unclaimed" && bonus.status === "active" && !isExpired
      };
    });
    ...
    const stats = {
      ...
      unclaimed: userBonuses.filter(b => b.userStatus === "unclaimed" && !b.isExpired).length,
      claimed: userBonuses.filter(b => b.userStatus === "claimed").length,
      expired: userBonuses.filter(b => b.isExpired).length,
      totalUnclaimedAmount: userBonuses
        .filter(b => b.userStatus === "unclaimed" && !b.isExpired)
...
// @route   POST /api/user/cash-bonus/claim/:bonusId
// @desc    Claim cash bonus for user
// @access  Private (User only)
user_route.post("/cash-bonus/claim/:bonusId", ensureAuthenticated, async (req, res) => {
  try {
    ...
    // Find bonus
    const bonus = await CashBonusModel.findById(bonusId);
    if (!bonus) {
      return res.status(404).json({
        success: false,
        message: "Bonus not found"
    ...
    if (!userBonus) {
      return res.status(404).json({
        success: false,
        message: "You are not assigned to this bonus"
      });
    }
    ...
    const transaction = new transaction_model({
      transaction_id: `CASH-BONUS-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
      customer_id: userId,
      customer_name: user.username,
      customer_email: user.email,
      payment_type: "bonus",
      ...
      customer_phone:user.phone,
      status: "success",
      transaction_note: `${bonus.title} bonus claimed`,
    ...
    res.status(200).json({
      success: true,
      message: `${bonus.title} bonus claimed successfully! ${bonus.amount} BDT added to your balance.`,
      data: {
        bonusId: bonus._id,
